...
describe('splitLines', () => {
  it('should split on newlines and drop empty lines', async () => {
    const source = fromBuffers(Buffer.from('a\n\nb\n', 'utf8'));

    expect(await collect(splitLines(source, NO_LINE_LIMIT))).toEqual(['a', 'b']);
  });

  it('should strip a trailing carriage return', async () => {
    const source = fromBuffers(Buffer.from('a\r\nb\r\n', 'utf8'));
  ...
  it('should join a line split across two chunks', async () => {
  ...
  it('should yield many lines from one chunk in order', async () => {
    const source = fromBuffers(Buffer.from('a\nb\nc\nd\n', 'utf8'));

    expect(await collect(splitLines(source, NO_LINE_LIMIT))).toEqual(['a', 'b', 'c', 'd']);
  });

  it('should not corrupt a two-byte character split across a chunk boundary', async () => {
  ...
  it('should not corrupt a four-byte emoji split across a chunk boundary', async () => {
  ...
  it('should raise when a single unterminated line exceeds the byte budget', async () => {
  ...
  it('should not raise when many short lines together exceed the budget', async () => {
    const source = fromBuffers(Buffer.from('aaaa\nbbbb\ncccc\n', 'utf8'));

    expect(await collect(splitLines(source, 6))).toEqual(['aaaa', 'bbbb', 'cccc']);
  });

  it('should raise when a newline-terminated line exceeds the budget within a single chunk', async () => {
